...
## 6、为什么要使用线程池（线程池的作用）,线程池的初始化参数，核心线程数数量设置
...
**对于CPU密集型的运算，多线程本质上是为了提升多核CPU的利用率，对一个n核的CPU，每个核心对应一个线程，所以理论上创建n个线程就可以了，创建再多的线程也只是增加线程切换的成本，不过实际上线程的数量一般会设置为CPU的核心数+1个，这样的话当线程因为偶尔的内存页失效或其他原因导致阻塞时，这个额外的线程就可以顶上，从而保证CPU的利用率。最佳线程数 = CPU核数 + 1**

**对于IO密集型的运算，如果CPU计算时间和IO操作时间是1:1，那么2个线程是最合适的，如果CPU计算时间和IO操作时间是1:2，那么3个线程是最合适的，CPU在ABC三个线程之间切换，对于线程A，当CPU从BC切换回来时线程A正好执行完IO操作，这样CPU和IO设备的利用率都达到了百分之百。最佳线程数 = CPU核数 * (1 + (IO耗时 / CPU耗时))**
...
## 8、JVM调优
**JVM优化是不得已的手段，大多数的java应用不需要进行JVM优化，优化代码和架构是更有效的手段**。比如如果内存不够用，检查代码是不是有地方发生了内存泄露，或者用合适的设计模式重构代码，或者直接加内存硬件；如果吞吐量不够可以考虑采用分布式的架构，或者添加缓存之类的。

影响最大的两个调优方法：

1、堆内存优化：可以通过将设置堆内存的初始大小等于堆内存的最大大小，可以避免JVM运行时进行内存扩展。同时调大堆内存，这样可以减少新生代和老年代垃圾回收的频率。

2、更换合适的垃圾回收器：截止到JDK1.8一共有七种不同垃圾回收器，可以根据具体的情况选择不同的垃圾回收器。
   （1）Serial，串行垃圾回收器，作用在新生代，使用的是标记-复制算法，特点是响应速度优先，适用于单CPU环境下的Client模式。
   （2）ParNew，并行垃圾回收器，作用在新生代，使用的是标记-复制算法，特点是响应速度优先，适用于多CPU环境下Server模式下与CMS配合使用。
   （3）Parallel，并行垃圾回收器，作用在新生代，使用的是标记-复制算法，特点是吞吐量优先，适用于后台运算而不需要太多交互的场景。
   （4）SerialOld，串行垃圾回收器，作用在老年代，使用的是标记-整理算法，特点是响应速度优先，适用于单CPU环境下Client模式。
   （5）ParallelOld，并行垃圾回收器，作用在老年代，使用的是标记-整理算法，特点是吞吐量优先，适用于后台运算而不需要太多交互的场景。
   （6）CMS，并发垃圾回收器，作用在老年代，使用的是标记-清除算法，特点是响应速度优先，适用于互联网或B/S业务。
   （7）G1，并发、并行垃圾回收器，作用在新生代和老年代，使用的是标记-复制算法、标记-整理算法，特点是响应速度优先，适用于服务端的应用。

   经验上来说：
   （1）小内存应用上CMS大概率会优于G1；大内存应用上，G1大概率会优于CMS。这个临界点大概在6G到8G之间。
   （2）如果想要最小化地使用内存和并行开销，可以选择SerialOld（老年代）和Serial（新生代）。
   （3）如果想要最大化应用程序的吞吐量，可以选择ParallelOld（老年代）和Parallel（新生代）。
   （4）如果想要最小化GC的中断或停顿时间，可以选择CMS（老年代）和ParNew（新生代）。
...
## 16、乐观锁，悲观锁，排他锁（互斥锁，独占锁），共享锁，读写锁，统一锁，分段锁，自旋锁，偏向锁，轻量级锁，重量级锁
...
首先要明确一个概念，保证缓存的一致性其实是一个伪命题，使用缓存本身就是为了应付高吞吐量的场景，而强一致性又得需要加锁来实现，这两者本身就是相互矛盾的，就比如说base定理，本身也只要求最终一致即可，是允许有一个“软状态”也就是短暂的不一致的。我们只能说尽可能的保证数据最终一致即可。

可以通过以下几个方式共同实现：

1. 设置缓存有效期。（对数据一致性要求不高情况，如果要求过高只能摒弃缓存了）
2. 做增删改的时候要通知redis做删除操作。（大部分情况可以做到一致。但是并发的情况可能会不一致，还得依托缓存有效期）
3. 延时双删。（能够解决并发问题，但是不能完全解决。）

mysql的数据和redis缓存中的数据，在多线程操作的时候有可能数据不一致，我们可以采用延时双删模式解决这个问题。延时双删，就是指线程在修改mysql数据的时候同时删除一下redis，延迟0.1秒再删除一下redis。这样可以解决掉99%的不一致问题。剩下的1%交给redis里面的定时器。因为我们放入到redis的数据都会给一个有效时长。
...
## 3、排查异常CPU飙高问题
1、使用top命令确认CPU过高的java进程。
2、通过“top -Hp 进程PID”查看该进程的所有线程情况，找到CPU过高的线程。（-H代表所有线程占用资源情况，-p指定进程。）
3、将线程PID转换为十六进制。
4、使用jstack来查看该java进程当前时间点所有的堆栈信息，然后使用grep来过滤出目标线程的堆栈信息，指令为“jstack 进程PID | grep 线程PID -A 后几条”。
5、拿到了导致CPU飙高的线程的堆栈信息就可以找到调用的具体代码，然后排查解决问题即可。

## 4、排查内存泄露问题
1、使用top命令确认内存占用过高的java进程。
2、使用jmap命令来生成该java进程当前时间点的堆转储文件，用于分析程序的内存使用情况，指令为“jmap -dump:format=b,file=转储文件名 进程PID”。
3、使用jhat命令分析堆转储文件“jhat 转储文件名”，jhat会启动一个http服务器，通过浏览器查看分析结果，在分享结果中可以查到java对象的详细信息包括对象数量、类信息、引用关系等。
4、拿到了对象信息就可以找到对应的对象调用代码，然后排查解决问题即可。