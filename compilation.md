...
​	**快照读**：一致性读也被成为快照读，使用普通的select语句进行查询时就会生成快照进行快照读，快照读不会上锁，根据可见性来决定读取该行记录最新版本还是旧版本。
...
​	在执行update或者delete操作时，会将每个操作记录到undoLog中，每一条记录中都会记录一个roll_pointer的引用信息指向该条数据的上一个undoLog记录，这样就会构成一条数据的版本链体现了该条数据的所有变更。

​	但是在查询的时候具体选择哪一个版本呢？这就需要用到ReadView来实现了。ReadView（读视图）顾名思义就是一个视图，在事务select
