...
## **为什么要使用Redis，Redis基本数据类型、数据结构，Redis的持久化机制。**
...
1. string应用最广，缓存数据可以使用字符串，我们将java的对象转换为json串存储到redis里面。
...
​		**缺点：**RDB快照是周期性的全量持久化，可能导致某个时间点之后的数据丢失；在保存快照时，Redis服务器会阻塞，可能对系统性能造成影响；
...
​		将单一的Redis更改为Redis Cluster（Redis集群）结构，在每一个Redis Node中都缓存一份热key，通过代码使得每次请求均匀的覆盖到各个Node中来缓解单一节点的热key问题。
