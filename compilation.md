...
​	在扩容时，会先创建一个新数组，长度是原来的2倍；之后遍历原数组，按照高低位分开进行hash计算在新数组的下标位置；按照尾插法插入数据，1.7之前使用的是头插法，这在并发的情况下扩容可能会导致死链问题（也就是环形列表），**但是要注意，即便尾插法解决了并发扩容死链问题但是HashMap依然是线程不安全的！**
...
​				（5）当幸存区对象按照年龄大小排序之后，占空间百分之五十后面的对象放入老年代。
